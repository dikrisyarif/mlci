  ...
  try {
    if (!db) {
      ...
      if (db && db._db && db._db.filename) {
        ...
        console.log('[DB][initDb] Database path:', db.filename);
      } else {
        console.log('[DB][initDb] Database path: [unknown]');
      }
...
export const getDb = async () => {
  while (typeof isCleaningUp !== 'undefined' && isCleaningUp) {
    await wait(100);
  }
  try {
    if (!db || db._closed) {
...
export const executeWithLog = async (operation, sql, params = [], useTransaction = false) => {
  try {
    ...
    return await queueOperation(async () => {
      ...
      try {
        if (useTransaction) {
          // Hindari nested transaction: cek apakah sudah dalam transaksi
          // Asumsi: tidak ada transaksi aktif, kita mulai manual
          ...
          try {
            ...
            // Rollback hanya jika transaksi sudah dimulai
...
export const queueOperation = async (operation, description) => {
  ...
  // Tunggu jika sedang force-cleanup
  while (isCleaningUp) {
    logDbOperation(operationId, 'waiting-for-cleanup');
    await wait(100);
  }
